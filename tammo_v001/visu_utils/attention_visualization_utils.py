...
def show_self_attention(
    ...
    for i, self_attention_map in enumerate(self_attention_maps):        

        image = white_image.resize((relevnace_res ** 2, relevnace_res ** 2))
        image = np.array(image)

        image_relevance = self_attention_map.reshape(1, 1, self_attention_map.shape[-1], self_attention_map.shape[-1])
        ...
        vis = np.array(Image.fromarray(vis).resize((256, 256)))
        
        # split_imgs.append(image)
        # image = text_under_image(image, decoder(int(tokens[i])))        
        vis = text_under_image(vis, prompts[i])
        images.append(vis)
