...
def fn_show_attention(
    ...
    for i in indices:
        ...
        for coord_x, coord_y in topk_coord_list:
            ...
            # NOTE 내가 일단 확인할 것은 self attention map의 avg가 아닌 한장 한장의 결과
            # 다른 code돌릴때 참고해야함 원래는 crtl tab으로 행간 for 문 밖으로 !! FIXME
    ...
    # for tensor_cross_attention in cross_attention_map_list:
    #     cross_attention_map_numpy.append(tensor_cross_attention.cpu().detach().numpy())
    
    # for tensor_self_attention in self_attention_map_numpy:
    #     self_attention_map_numpy.append(tensor_self_attention.cpu().detach().numpy())
...
def fn_select_self_attention(
    ...
    for i in indices:
        ...
        # if resize:            
        #     norm_self_attention_map_per_token = norm_self_attention_map_per_token.unsqueeze(0).unsqueeze(0)
        #     resized_samap_per_token = F.interpolate(norm_self_attention_map_per_token, size=(256, 256), mode='bilinear')
        #     resized_samap_per_token = resized_samap_per_token.squeeze(0).squeeze(0)
        #     self_attention_map_list.append(resized_samap_per_token)
