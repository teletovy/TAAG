...
from utils.attn_utils import fn_show_attention, fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask, fn_select_self_attention
...
class StableDiffusionInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    ...
    def _perform_iterative_refinement_step(
        ...
        while cross_attn_loss > target_loss or self_attn_loss > target_self_loss:
            ...
            if logger is not None:
                logger.info(f"\t Try {iteration}. cross loss: {cross_attn_loss:0.4f}. self loss: {self_attn_loss:0.4f}. clean loss: {clean_loss:0.4f}")
            else:
        ...
        joint_loss, cross_attn_loss, clean_loss, self_attn_loss = self.fn_augmented_compute_losss(indices=indices, K=1)
        if logger is not None:
            logger.info(f"\t Finished with loss of: {cross_attn_loss:0.4f}")
        else:
    ...
    def __call__(
        ...
        K: int = 1,        
        logger: Optional[Any] = None,
        # segmentation_self_attn: Optional[str] = False
    ):
        r"""
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    for latent, index, text_embedding in zip(latents, indices, text_embeddings):
                        ...
                        # if i % cfg.attn_sampling_rate == 0 or i == 25 or i == 15 or i == 5:
                        ...
                        if i < cfg.max_iter_to_alter and (i == 10 or i == 20) and (cross_attn_loss > 0.2 or self_attn_loss > 0.3) and not cfg.run_sd and True:
                        ...
                        if i < cfg.max_iter_to_alter and not cfg.run_sd:
                            ...
                            logger.info(f"Iteration {i:02d} - cross loss: {cross_attn_loss:0.4f} - self loss: {self_attn_loss:0.4f}")

                        updated_latents.append(latent)

                    latents = torch.cat(updated_latents, dim=0)

                #NOTE 0930 object shape mask보려고 추가함
        ...
        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), {
            'cross': cross_attention_map_list, 'masks': self.masks_per_token, 'nsfw':has_nsfw_concept[0]
        } #NOTE 1122 nsfw block 추가
