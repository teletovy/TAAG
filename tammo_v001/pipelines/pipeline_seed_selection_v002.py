...
class SDPlusSeedSelectV002(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        cfg: Dict,                
        logger: Any, #NOTE 09/25 from now logger is essential part of pipelines
        seed_group: List[int], #NOTE 09/25 sampled seed
        height: Optional[int] = None,
        width: Optional[int] = None,
        negative_prompt: Optional[Union[str, List[str]]] = None,
        num_images_per_prompt: int = 1,
        eta: float = 0.0,
        # generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None, #NOTE 09/25 mine generator didn't need
        ...
        #NOTE 09/25 애초에 필요없는 부분일 수도 있지만, 현 위치에서 initno loop 이후로 옮김
        # extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)
        ...
        # 8. initno & Seed selection
        if cfg.seed_select_loss: seed_select_pool = [] #NOTE 09/25 loss seed selection v002
        min_seed_initno_loss = 1000
        for select_iteration, seed in tqdm(enumerate(seed_group)):
            ...
            # if cfg.run_initno: #NOTE 09/25 seed select v002는 initno가 필수 기 떄문에 flag 제거
            max_round = 5
            initno_loss, latents = self.fn_initno_loop(                
                latents=latents, max_round=max_round, token_indices=token_indices, text_embeddings=text_embeddings, cfg=cfg, device=device,
                generator=generator, eta=eta, do_classifier_free_guidance=do_classifier_free_guidance, batch_size=batch_size,
                num_images_per_prompt=num_images_per_prompt, num_channels_latents=num_channels_latents, height=height, width=width, dtype=prompt_embeds.dtype
            )
            logger.info(f'seed number {seed} with initno loss {initno_loss.item():0.4f}')
            if initno_loss.item() < min_seed_initno_loss:
                seed_select_pool.append((initno_loss.item(), seed))
                min_seed_initno_loss = initno_loss.item()                
            seed_select_pool.sort()
            if len(seed_select_pool) > 3:
                seed_select_pool.pop(-1)

        seed_numbers = []
        for _, seed_number in seed_select_pool:
        ...
        for seed_number in seed_numbers:
            ...
            final_seed_loss, latents = self.fn_initno_loop(
                latents=latents, max_round=max_round, token_indices=token_indices, text_embeddings=text_embeddings, cfg=cfg, device=device,
                    generator=generator, eta=eta, do_classifier_free_guidance=do_classifier_free_guidance, batch_size=batch_size,
                    num_images_per_prompt=num_images_per_prompt, num_channels_latents=num_channels_latents, height=height, width=width, dtype=prompt_embeds.dtype
            )
            logger.info(f'Start with seed number {seed_number}, initno loss {final_seed_loss.item():0.4f}')
            # 6. Prepare extra step kwargs. TODO: Logic should ideally just be moved out of the pipeline
            extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)
            ...
            with self.progress_bar(total=cfg.num_inference_steps) as progress_bar:
                ...
                for i, t in enumerate(timesteps):                
                    ...
                    if i == cfg.max_shape_iter - 1:
                        ...
                        for k, v in self.masks_per_token.items():
                            ...
                            anormal_masks.append(cnt) #NOTE as you know normal part should be 2 for each subjects mask
                            os.makedirs(f'{save_path}/{seed_number}', exist_ok=True)
                            cv2.imwrite(f'{save_path}/{seed_number}/{k}_{50 - i}steps_mask.jpg', v * 255)
                    ...
                    if self.masks_per_token is not None:
                        for anormal_mask in anormal_masks:
                            if anormal_mask > 2 or anormal_mask == 1: 
                                mask_anormaly = True
                    
                    if mask_anormaly: 
                        if logger is not None:
                            logger.info(f'anormaly {mask_anormaly} at seed {seed_number}! ')
                        break
                ...
                # here is out of denoising loop but still in seed loop
                if not mask_anormaly:
                    break
    ...
    def fn_initno_loop(self, latents, max_round, token_indices, text_embeddings, cfg, device, generator, eta, do_classifier_free_guidance,
            batch_size, num_images_per_prompt, num_channels_latents, height, width, dtype):
        ...
        with torch.enable_grad():
            ...
            if optimized_latents_pool[0][4] is True:
                ...
                return optimized_latents_pool[0][0], latents
