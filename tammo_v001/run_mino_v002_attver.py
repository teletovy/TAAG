...
from pipelines.pipeline_mino_v002_att import SDMINOv002Att
from visu_utils.attention_visualization_utils import show_cross_attention
import cv2
from utils.code_utils import copy_src_files, get_logger, get_indices, get_time_str, get_iou_of_mask, get_texture_indices
...
def main(cfg):
    ...
    pipe = SDMINOv002Att.from_pretrained(cfg.model_id).to("cuda")    
    ...
    for prompt in PROMPTS(cfg.subset_key):        
        ...
        #NOTE 1109 att biniding 고려 ver.

        if cfg.subset_key == 'animals':
            token_indices = get_indices(prompt, logger)
            texture_indices = None            
        elif cfg.subset_key == 'objects' or cfg.subset_key == 'animals_objects':
            token_indices, texture_indices = get_texture_indices(prompt, logger, cfg.subset_key)
        else:
            ValueError(f'invalid subset {cfg.subset_key}')
        
        logger.info(f"token indices:{token_indices}")            
        logger.info(f"texture indices:{texture_indices}")            
...
if __name__ == '__main__':
    cfg_path = '/home/initno1/GSN/mino_v002/conf/conf_mino_v002_att.yaml'
    main(cfg_path)
